...
- Review memory is saved locally in `match_memory.json`:
  ...
  - export uses the current effective session memory (confirmed + rejected), not just the default file
...
- Unit price is now a display-only layer that runs after grouping:
  ...
  - optional AI fallback can assist only when rule-based unit parsing is uncertain (requires `OPENAI_API_KEY`)
  - user corrections always win over AI suggestions
